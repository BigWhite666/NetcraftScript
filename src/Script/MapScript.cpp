...
void MapWorker::setParams(float targetX, float targetY, float targetZ, int loopCount, const QList<HWND>& windows) {
    m_targetX = targetX;
    m_targetY = targetY;
    m_targetZ = targetZ;
    ...
    try {
        for (HWND hwnd : m_windows) {
            ...
            while (m_running) {
                ...
                // 计算与目标点的距离
                float dx = currentX - m_targetX;
                float dy = currentY - m_targetY;
                float dz = currentZ - m_targetZ;
                ...
                if (distance > 2.0f) {
                    // 瞄准目标点并移动
                    AimTarget(hwnd, m_targetX, m_targetY, m_targetZ);
                    if (!isForwardKeyDown) {
                        DM->KeyDown('W');  // 按住前进键
                ...
                // 高度控制
                float heightDiff = m_targetZ - currentZ;
...
void MapScript::start(float targetX, float targetY, float targetZ, int loopCount, const QList<HWND>& windows) {
    ...
    // 设置参数并启动线程
    m_worker->setParams(targetX, targetY, targetZ, loopCount, windows);
    m_thread->start();
